...
> **MDT note:** For repo-specific trigger scenarios (test pipeline, JSONL artifacts, smoke tests, CI validators), see also: `.claude/rules/context-mode.md`
...
- **`ctx_execute(language, code)`** — run code or shell commands in a sandbox; stdout becomes your result
- **`ctx_execute_file(path, language, code)`** — same, but the file at `path` is pre-loaded as `FILE_CONTENT` in your code
